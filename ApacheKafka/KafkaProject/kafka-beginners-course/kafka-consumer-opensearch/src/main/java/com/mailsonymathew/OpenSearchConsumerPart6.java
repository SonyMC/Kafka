...
public class OpenSearchConsumerPart6 {
    ...
    // Function rti extract id from json data using gson library
    private static String extractId(String json) {
        ...
        return JsonParser.parseString(json)
                ...
                .get("meta")
                .getAsJsonObject()
                .get("id")  // get value of 'id' node under 'meta'
                .getAsString();
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        Logger log = LoggerFactory.getLogger(OpenSearchConsumerPart6.class.getSimpleName());
        ...
        try (openSearchClient; consumer) {  // pass in the openSearchClient & Kafka consumer as arguments in the try block
            ...
            while (true) {
                ...
                // Note : A bulk request holds an ordered IndexRequests, DeleteRequests and UpdateRequests and allows to executes it in a single batch. Note that we only support refresh on the bulk request not per item.
                ...
                for (ConsumerRecord<String, String> record : records) {
                    ...
                    try {
                        // Putting in a try catch to skip the concrete object exception which will happen otheewise
                        ...
                        IndexRequest indexRequest = new IndexRequest("wikimedia")
                                ...
                                .id(id); // Pass the extracted id to the index request


                        // Now access the openSearchClient to add the record
                        //IndexResponse response = openSearchClient.index(indexRequest, RequestOptions.DEFAULT);

                        // Add the Index Request  to the Bulk Request instead of creating an Index Response for accessings the openSearchClient
                ...
                if (bulkRequest.numberOfActions() > 0) {  // execute only if there is data to be actioned in teh Bulk Request
                    // Create Bulk Response for accessing the openSearchClient
                    BulkResponse bulkResponse = openSearchClient.bulk(bulkRequest, RequestOptions.DEFAULT);
                    log.info("Inserted " + bulkResponse.getItems().length + " record(s).");

                    try {
                        Thread.sleep(1000);  // Introduce a delay of 1 second in this thread to increase our chances of getting a bulk action
                ...
                // commit offsets after the batch is consumed .
                // This is sn 'atleast once commit' as we commit only after processign the entire batch successfullys
